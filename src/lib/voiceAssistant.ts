...
export function processQuestion(raw: string): string {
  ...
  if (/(vendas?|faturamento|faturou|faturamos|vendeu|vendemos|receita).*(hoje|do dia|de hoje|diaria)/.test(q) ||
    ...
    const lucro = todayIncome - todayExpense
    ...
    msg += '.'
    if (todayExpense > 0) msg += ` Despesas: ${fmt(todayExpense)}. Lucro líquido: ${fmt(lucro)}.`
    return msg
  }

  // --- VENDAS ONTEM ---
  if (/(vendas?|faturamento|faturou|vendeu|receita).*(ontem)/.test(q) ||
      /ontem.*(vendas?|faturou|vendeu)/.test(q)) {
  ...
  // --- VENDAS DA SEMANA ---
  if (/(vendas?|faturamento|receita).*(semana|semanal)/.test(q) ||
      /(semana|semanal).*(vendas?|faturamento|receita)/.test(q)) {
  ...
  // --- VENDAS DO MÊS ---
  if (/(vendas?|faturamento|receita).*(mes|mensal)/.test(q) ||
      /(mes|mensal).*(vendas?|faturamento|receita)/.test(q) ||
      /quanto.*faturamos.*(mes|mensal)/.test(q)) {
  ...
  // --- DESPESAS ---
  if (/(despesa|gasto|gastos|gastou|gastamos|custo).*(hoje|do dia)/.test(q) ||
      /(hoje|do dia).*(despesa|gasto|gastos|custo)/.test(q)) {
  ...
  if (/(despesa|gasto|gastos|custo).*(mes|mensal)/.test(q) ||
      /(mes|mensal).*(despesa|gasto|gastos|custo)/.test(q)) {
  ...
  // --- LUCRO ---
  if (/(lucro|lucros|ganho|ganhos|resultado|margem).*(hoje|do dia)/.test(q) ||
      /(hoje|do dia).*(lucro|ganho|resultado)/.test(q)) {
    ...
    return `Lucro de hoje: ${fmt(lucro)}. Receita: ${fmt(inc)}, despesas: ${fmt(exp)}.`
  }

  if (/(lucro|lucros|ganho|resultado|margem).*(mes|mensal)/.test(q) ||
      /(mes|mensal).*(lucro|ganho|resultado)/.test(q)) {
  ...
  // --- COMPRAS (fornecedor) ---
  if (/(compras?|comprou|compramos|fornecedor).*(hoje|do dia)/.test(q) ||
      /(hoje|do dia).*(compras?)/.test(q)) {
  ...
  if (/(compras?|comprou|compramos|fornecedor).*(mes|mensal)/.test(q) ||
      /(mes|mensal).*(compras?)/.test(q)) {
  ...
  // --- PEDIDOS PENDENTES ---
  if (/(pedido|pedidos|encomenda).*(pendente|aberto|por fazer|em espera)/.test(q) ||
      /(pendente|aberto).*(pedido|encomenda)/.test(q) ||
      /quantos? pedidos?/.test(q)) {
