...
/** Turno aberto do utilizador indicado (cada funcionário/dispositivo tem o seu próprio turno) */
export function getOpenShift(userId?: string): ShiftSession | null {
  if (!userId) return null
  return loadShifts().find(s => !s.closed_at && s.opened_by_id === userId) || null
}
...
export function TurnoTab({ movements }: { movements: CashMovement[] }) {
  ...
  const [openShift, setOpenShift] = useState<ShiftSession | null>(() => getOpenShift(user?.id))
  ...
  const reload = useCallback(() => {
    ...
    setShifts(fresh)
    setOpenShift(fresh.find(s => !s.closed_at && s.opened_by_id === user?.id) || null)
  ...
  const handleOpen = (e: React.FormEvent) => {
    e.preventDefault()
    if (openShift) { toast.error('Já tem um turno aberto'); return }
  ...
  const otherOpenShifts = shifts.filter(s => !s.closed_at && s.id !== openShift?.id)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ═══ OUTROS TURNOS ABERTOS (outros funcionários/dispositivos) ═══ */}
      {otherOpenShifts.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
          <h3 className="text-sm font-bold text-blue-800 mb-3 flex items-center gap-2">
            <User className="w-4 h-4" /> Outros turnos abertos agora ({otherOpenShifts.length})
          </h3>
          <div className="space-y-2">
            {otherOpenShifts.map(s => (
              <div key={s.id} className="bg-white rounded-xl px-4 py-2.5 flex items-center justify-between border border-blue-100">
                <div>
                  <p className="text-sm font-semibold text-gray-700">{s.opened_by || 'Desconhecido'}</p>
                  <p className="text-xs text-gray-400">desde {fmtTime(s.opened_at)} ({fmtDuration(s.opened_at)})</p>
                </div>
                <span className="text-xs font-bold text-blue-600 bg-blue-100 px-2 py-1 rounded-full">Aberto</span>
              </div>
            ))}
          </div>
        </div>
