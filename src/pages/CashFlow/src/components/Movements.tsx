...
import { postAutoEntryForMovement, removeJournalEntry } from '../../../../lib/accounting'
...
export function Movements({ movements, setMovements, accounts, setAccounts, categories }: MovementsProps) {
  ...
  // Mapeia o tipo de conta do Fluxo de Caixa para a conta PGC-AO (11 Caixa, 12 Depósitos à Ordem)
  const accountCode = (name: string) => accounts.find(a => a.name === name)?.type === 'cash' ? '11' : '12'
  ...
  const add = () => {
    ...
    postAutoEntryForMovement(mv.id, mv.date, mv.description, mv.type, mv.amount, accountCode(mv.account), mv.accountTo ? accountCode(mv.accountTo) : undefined)
  ...
  const remove = (mv: CashMovement) => {
    ...
    removeJournalEntry(`auto-mov-${mv.id}`)
    setAccounts(prev => prev.map(a => {
  ...
  const saveEditValue = (mv: CashMovement) => {
    ...
    removeJournalEntry(`auto-mov-${mv.id}`)
    postAutoEntryForMovement(mv.id, mv.date, mv.description, mv.type, newAmount, accountCode(mv.account), mv.accountTo ? accountCode(mv.accountTo) : undefined)
                  ...
                  <div className="flex-shrink-0 relative group/move hidden sm:block">
                    <select
                      ...
                      onChange={e => {
                        ...
                        if (isSupabaseReady()) syncCfMovements([updated])
                        removeJournalEntry(`auto-mov-${m.id}`)
                        postAutoEntryForMovement(m.id, m.date, m.description, m.type, m.amount, accountCode(newAcc), m.accountTo ? accountCode(m.accountTo) : undefined)
