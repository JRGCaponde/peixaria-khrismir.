...
import {
  ...
  Banknote, Smartphone, Building2, X, Clock, Check, ChevronDown, ChevronUp,
  Pencil,
} from 'lucide-react'
...
export default function CashFlow() {
  ...
  useEffect(() => {
    ...
    if (isSupabaseReady()) {
      pullAll().then(() => loadAll())
    } else {
      loadAll()
    }

    // Ouve eventos Realtime (outro dispositivo alterou dados) e pullAll
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'accounts'   && <AccountsTab accounts={accounts} setAccounts={setAccounts} movements={movements} setMovements={setMovements} categories={categories} />}
...
function calcBalance(accountName: string, movements: CashMovement[]): number {
  let balance = 0
...
function AccountsTab({ accounts, setAccounts, movements, setMovements, categories }: {
  accounts: Account[]; setAccounts: React.Dispatch<React.SetStateAction<Account[]>>
  movements: CashMovement[]; setMovements: React.Dispatch<React.SetStateAction<CashMovement[]>>
  categories: Category[]
}) {
  ...
  const saveEditMv = (mv: CashMovement) => {
    ...
    // 1. Actualizar movimentos — o useEffect persiste e sincroniza
    ...
    setAccounts(prev => prev.map(a => {
      if (!affectedNames.has(a.name)) return a
      // Simular a lista de movimentos actualizada
